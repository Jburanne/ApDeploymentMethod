...
class Decimation
{
  public:
    ...
    void free_memory();
    void init(int *ls_local_opt, int *ls_global_opt, lit *ls_unit_clause, int ls_unit_clause_count, int *ls_clause_lit_count, int *ls_clause_cardinality);
...
void Decimation::init(int *ls_local_opt, int *ls_global_opt, lit *ls_unit_clause, int ls_unit_clause_count, int *ls_clause_lit_count, int *ls_clause_cardinality)
{
    int v;
...
void Decimation::assign(int v, int sense)
{
    ...
    for (int i = 0; i < var_lit_count[v]; ++i)
    {
        ...
        if (sense == var_lit[v][i].sense) //fix v will make literal var_lit[v][i] be true
        {
            //clause_delete[c] = 1;
            ...
            {
            	//delete variable v from clause c
        ...
        ///fix v will make literal var_lit[v][i] be false
        for (int j = 0; j < clause_lit_count[c]; j++)
        {
            if (clause_lit[c][j].var_num == v)
            {
...
void Decimation::hunit_propagation()
{
    ...
    {
        sense = choose_sense(v); //randomly choose 0 or 1 as sense
    }
    assign(v, sense);
}
...
void Decimation::random_propagation1()
{
    int v, sense;
    v = unassigned_var[rand() % unassigned_var_count];
    sense = 2;
