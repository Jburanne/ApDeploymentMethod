...
void Satlike::allocate_memory()
{
	...
	fix_clause = new int[malloc_clause_length];
...
void Satlike::build_instance(char *filename)
{
	...
	fix_num = 0;
	fix_clause_num = 0;
	cout<<"before build, fix num:"<<fix_num<<endl;
	...
	while (getline(infile, line2))
	{
		...
		//cout<<"weight:"<<org_clause_weight[c]<<", thres:"<<clause_true_lit_thres[c]<<endl;
		...
		if (clause_lit_count[c] <= clause_true_lit_thres[c]) //kkk
		{
			//cout<<"lit_count:"<<clause_lit_count[c]<<", thre:"<<clause_true_lit_thres[c]<<endl;
			for(int i = 0;i < clause_lit_count[c];++i){
				unit_clause[unit_clause_count++] = clause_lit[c][i];
				if(org_clause_weight[c] == top_clause_weight && fixed[clause_lit[c][i].var_num] == 0){
					fixed[clause_lit[c][i].var_num] = 1; //fix variable, can not be picked
					fix_num++;
					//cout<<"fix num ++"<<endl;
				}		
			}
	...
	for (c = 0; c < num_clauses; ++c)
	{
		...
		for (i = 0; i < clause_lit_count[c]; ++i)
		{
			...
			++var_lit_count[v];
			if(fixed[v] == 1) fix_var_num++;
		}
		if(fix_var_num == clause_lit_count[c]){
			fix_clause[c] = 1;
			fix_clause_num++;
		}
	}
...
{
	//copy��ʼ���������㵱ǰÿ��clause��true literal�ĸ�����ÿ��var�ķ��� 
	int v, c;
	int j;
	...
	for (c = 0; c < num_clauses; ++c)
	{
		...
		&& (clause_lit_count[c] >= clause_true_lit_thres[c] || sat_count[c] < clause_lit_count[c]))
		{
			if (org_clause_weight[c] != top_clause_weight){
...
int Satlike::pick_var()
{
	...
	if (goodvar_stack_fill_pointer > 0)
	{
		...
		if (goodvar_stack_fill_pointer < hd_count_threshold) //greedy
		{
			...
			for (i = 1; i < goodvar_stack_fill_pointer; ++i)
			{
				...
				if(fixed[best_var] == 1) best_var = v;
				//cout<<"enter 1 ���� v --- "<<v<<endl;
	...
	if (hardunsat_stack_fill_pointer > 0)
	{
		//cout<<"enter 2 ����"<<endl;
		sel_c = -1;
		while(sel_c == -1 || fix_clause[sel_c] == 1){
...
int Satlike::pick_add_var(){
	int best_var = -1;
	int sel_c;
	if(rand()%100 < 30){
		sel_c = hardunsat_stack[rand() % hardunsat_stack_fill_pointer];
		while(fix_clause[sel_c] == 1){
			sel_c = hardunsat_stack[rand() % hardunsat_stack_fill_pointer];
		}
		while(best_var == -1 || fixed[best_var] == 1){
		...
		sel_c = hardunsat_stack[i];
		int v;
		for(int j = 0;j < clause_lit_count[sel_c];++j){
			v = clause_lit[sel_c][j].var_num;
			if(fixed[v] == 1) continue;
			//if(cur_soln[v] == 1) continue; 
			if(best_var == -1){
				best_var = v;
				continue;
			}
...
int Satlike::pick_hardunsat_var_randomly(){
	int sel_c = -1;
	while(sel_c == -1 || fix_clause[sel_c] == 1){
		sel_c = hardunsat_stack[rand() % hardunsat_stack_fill_pointer];
	}
	int best_var = -1;
	while(best_var == -1){
		int random_pos = rand() % clause_lit_count[sel_c];
		int v = clause_lit[sel_c][random_pos].var_num;
		if(score[v] > 0){
			best_var = v;
		}
	}
	...
	if(v < clause_lit_count[sel_c]) return v;
...
void Satlike::sat_one_softunsat(int step){
	int sel_c = softunsat_stack[rand() % softunsat_stack_fill_pointer];
	int pos = 0;
	while(index_in_softunsat_stack[sel_c] != -1 && pos < clause_lit_count[sel_c]){
		int v = clause_lit[sel_c][pos].var_num;
		if(fixed[v] == 1){
			pos++;
			continue;
		}
...
void Satlike::local_search_with_decimation(vector<int> &init_solution, char *inputfile)
{
	...
	for (tries = 1; tries < max_tries; ++tries)
	{
		...
		cout<<endl;
		cout<<"hard_unsat_nb:"<<hard_unsat_nb<<endl;
		cout<<"fix_num:"<<fix_num<<endl;
		cout<<"num_vars:"<<num_vars<<endl;
		while(hard_unsat_nb > 0){
		...
		for (step = 1; step < max_flips; ++step)
		{
			...
			{
				//cout<<"update local solution"<<endl;
...
bool Satlike::verify_sol()
{
	...
	for (c = 0; c < num_clauses; ++c)
	{
		...
		if (tem_clause_true_lit_count >= clause_true_lit_thres[c] || tem_clause_true_lit_count == clause_lit_count[c])
			flag = 1;
		/*
		if (tem_clause_true_lit_count >= clause_true_lit_thres[c])
			flag = 1;
...
void Satlike::increase_weights()
{
	...
	for (i = 0; i < hardunsat_stack_fill_pointer; ++i)
	{
		...
		for (lit *p = clause_lit[c]; (v = p->var_num) != 0; p++)
		{
			...
			{
				score[v] += h_inc;
				if (score[v] > 0 && already_in_goodvar_stack[v] == -1 && fixed[v] == 0)
				{
					already_in_goodvar_stack[v] = goodvar_stack_fill_pointer;
					mypush(v, goodvar_stack);
				}
